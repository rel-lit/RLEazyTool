...
def print_help() -> None:
    ...
    print("  回车      : 执行合并（c 模式下仅合并已选文件）")
    print("")
    print("【目录范围 this】（仅当前合并根目录；与 mod/exc 独立；配置见 merge_config.json）")
    print("  this      : 开关配置模式（进入后启用已保存范围；退出后合并不再应用，设置仍保留）")
    print("  this 0    : 保存深度 0（仅本层；仅 this 模式开启时合并生效）")
    print("  this N    : 保存最大深度 N（仅 this 模式开启时合并生效）")
